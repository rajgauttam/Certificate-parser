...
import com.cisco.nm.vms.api.certificate.parser.CertificateParser.CertificateType;
...
public class CertificateParserHelper {
    ...
    private static final String IDENTITY_CERTIFICATE_PATH = "ROOT/Certificate/";
    private static final String CA_CERTIFICATE_PATH = "ROOT/CA Certificate/";
    /**
    ...
    private  CertificateParser<KeyValuePairCertificateNode> caCertificateParser = null;
    private  CertificateParser<KeyValuePairCertificateNode> identityCertificateParser = null;

    private CertificateParserHelper(){
        caCertificateParser = CertificateParser.getInstance(CertificateType.CA);
        identityCertificateParser = CertificateParser.getInstance(CertificateType.IDENTITY);
    }

    public static CertificateParserHelper getInstance(){
        return new CertificateParserHelper();
    }

    public  List<CertificateInfoNode> getCaCertificateInfoNode(String certStr) throws NBIException{
        buildParsingRules(caCertificateParser);
        buildParsingRules(caCertificateParser, CA_CERTIFICATE_PATH);
    ...
    public  List<CertificateInfoNode> getIdentityCertificateInfoNode(String certStr) throws NBIException{
        buildParsingRules(identityCertificateParser);
        buildParsingRules(identityCertificateParser, IDENTITY_CERTIFICATE_PATH );
        KeyValuePairCertificateNode rootNode = identityCertificateParser.parse(certStr);
        return convertCertificateNodeToEntry(rootNode.getChildren());
    }

    private  List<CertificateInfoNode> convertCertificateNodeToEntry(List<KeyValuePairCertificateNode> certificateNodesList){
        List<CertificateInfoNode> certificateNodeEntries = new ArrayList<>();
        for (KeyValuePairCertificateNode certNode : certificateNodesList) {
            CertificateInfoNode CertificateInfoNode = new CertificateInfoNode();
            CertificateInfoNode.setKey(certNode.getDispKey());
            CertificateInfoNode.setValue(certNode.getValue());
            CertificateInfoNode.setChildrenList(convertCertificateNodeToEntry(certNode.getChildren()));
            certificateNodeEntries.add(CertificateInfoNode);
        }
    ...
    private static void buildParsingRules(CertificateParser<KeyValuePairCertificateNode> parser) {
        ...
        parser.getCertificateParserBuilder()
               ...
               .addKeyTransformation("Subject Name",  "Issued To")
